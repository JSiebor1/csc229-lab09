...
What is the value of the first triangle number to have over one hundred divisors?
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            triangleNumber += i;
            int count = countDivisors(triangleNumber);
            if (count > 100) { // more than 100 divisors
    ...
    public static int countDivisors(int n) {
        ...
        for (int i = 1; i <= sqrt; i++) {
            if (n % i == 0) {
                count += 2; // both i and n/i as divisors
            }
        }
        if (sqrt * sqrt == n) { // special case for perfect squares, would normally count it twice
            count--;
        }
        return count;
    }
